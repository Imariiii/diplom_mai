...
function getPairwise(result: ComparisonResult): PairwiseComparison[] {
  if (isPerTestResult(result)) return result.pairwise
  if (isSeriesResult(result)) {
    return Object.values(result.per_db).flatMap((s) => s.adjacent_level_tests)
  }
  return []
}

function getDescriptiveStats(result: ComparisonResult): Record<string, Record<string, MetricStatsBundle>> {
  if (isPerTestResult(result)) {
    return { [result.test.id]: result.descriptive_stats }
  }
  if (isSeriesResult(result)) {
    const m: Record<string, Record<string, MetricStatsBundle>> = {}
    for (const t of result.tests) {
      const bundles: Record<string, MetricStatsBundle> = {}
      for (const [dbKey, summary] of Object.entries(result.per_db)) {
        for (const [levelId, bundle] of Object.entries(summary.descriptive_stats_by_level)) {
          const key = `${dbKey}__${levelId}`
          bundles[key] = bundle
        }
        if (summary.trajectory.length > 0) {
          const last = summary.trajectory[summary.trajectory.length - 1]
          if (summary.descriptive_stats_by_level[last.level_id]) {
            bundles[dbKey] = summary.descriptive_stats_by_level[last.level_id]
          }
        }
      }
      m[t.id] = bundles
...
function getTests(result: ComparisonResult): { id: string; name: string }[] {
...
export function ExecutiveSummary({ result }: ExecutiveSummaryProps) {
  ...
  const largeEffects = pairwise.filter(
  ...
  const showTrends = isSeriesResult(result) && result.tests.length >= 3
      ...
      <div className="grid gap-0 lg:grid-cols-[1.4fr_1fr]">
        <div className="space-y-4 p-6 lg:p-8">
          ...
          <h2 className="text-pretty text-xl font-semibold leading-snug tracking-tight md:text-[1.35rem]">
            {verdict || "Статистически значимых различий не обнаружено."}
          </h2>
...
function SeriesTrendsStrip({ result }: { result: ComparisonResult }) {
  ...
  const trends = useMemo(() => {
    const series: { key: string; label: string; unit: string; values: number[] }[] = []
    for (const [dbKey, summary] of Object.entries(result.per_db)) {
      const label = result.db_key_labels[dbKey] || summary.db_label || dbKey
        ...
        {trends.map((t) => (
          <div key={t.key} className="space-y-1">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-mono text-muted-foreground tabular-nums">{t.unit}</span>
            </div>
...
function MiniSparkline({ values }: { values: number[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="h-6 w-full text-primary" preserveAspectRatio="none">
...
function IntensityBadge({ intensity }: { intensity: "strong" | "moderate" | "weak" | "neutral" }) {
  ...
  return <Badge variant="outline" className={map[intensity].cls}>{map[intensity].label}</Badge>
}

function WinnerPill({ icon, label, name, db, value }: { icon: React.ReactNode; label: string; name: string; db: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 rounded-lg border border-border bg-muted/30 px-3 py-2.5">
      <div className="flex items-center gap-2.5 min-w-0">
        <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-success/15 text-success">{icon}</div>
        <div className="min-w-0">
          <p className="text-[11px] uppercase tracking-wider text-muted-foreground">{label}</p>
          <p className="truncate text-sm font-medium">{name}<span className="text-muted-foreground"> · {db}</span></p>
        </div>
      </div>
...
function KpiTile({ icon, label, primary, secondary, progress, tone }: {
  icon: React.ReactNode; label: string; primary: string; secondary: string; progress?: number;
  tone: "primary" | "accent" | "success" | "neutral"
}) {
  const toneMap = { primary: "text-primary", accent: "text-warning", success: "text-success", neutral: "text-muted-foreground" } as const
  const barColor = { primary: "bg-primary", accent: "bg-warning", success: "bg-success", neutral: "bg-muted-foreground/60" } as const
  ...
  return (
    <div className="bg-card p-4 md:p-5">
      ...
      <p className="mt-2 font-mono text-2xl font-semibold tabular-nums leading-none">{primary}</p>
      <p className="mt-1 truncate text-xs text-muted-foreground">{secondary}</p>
      {progress != null && (
        <div className="mt-3 h-1 w-full overflow-hidden rounded-full bg-muted">
          <div className={`h-full rounded-full ${barColor[tone]}`} style={{ width: `${Math.min(100, Math.max(4, progress * 100))}%` }} />
        </div>
      )}
    </div>
  )
...
function findBestMetric(
  result: ComparisonResult, type: "throughput" | "latency", direction: "higher" | "lower"
): { testName: string; value: number; dbKey: string } | null {
  let best: { testName: string; value: number; dbKey: string } | null = null

  if (isPerTestResult(result)) {
    for (const [dbKey, bundle] of Object.entries(result.descriptive_stats)) {
      const val = type === "throughput" ? bundle.throughput?.mean : bundle.latency_ms?.mean
      if (val == null) continue
      if (!best || (direction === "higher" && val > best.value) || (direction === "lower" && val < best.value)) {
        best = { testName: result.db_key_labels[dbKey] || dbKey, value: val, dbKey }
      }
    }
  } else if (isSeriesResult(result)) {
    for (const [dbKey, summary] of Object.entries(result.per_db)) {
      for (const tp of summary.trajectory) {
        const val = type === "throughput" ? tp.throughput_mean : tp.latency_mean
        if (val == null) continue
        if (!best || (direction === "higher" && val > best.value) || (direction === "lower" && val < best.value)) {
          best = { testName: result.db_key_labels[dbKey] || summary.db_label || dbKey, value: val, dbKey }
        }
      }
    }
  }
